...
public interface UtenteRepositoryJpa extends JpaRepository<Utente,Long> {
    ...
    @Query("select u from Utente u left join u.followers left join u.followed where u.id=:id")
    Optional<Utente> findUserWithFollowersAndFollowedById(Long id);

    Boolean existsByUsername(String username);

    @Query("select u from Utente u left join u.posts left join u.followed left join u.followers where u.id=:id")
